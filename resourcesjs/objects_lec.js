...
(function() {
    ...
    let kamea = {
        ...
        colors: ["Black", "White"],
        species: "Orca",
        date_of_birth: new Date("12/6/2013") //final property should not have a comma after it.
    };

    // We can also define property values to an object these two ways!
    kamea.age = 6; //.property is the most common way to call or add a field
    kamea["weight_in_lbs"] = 2400; // the square brackets
    ...
    // While we store the different forecasts in an array to hold the information for the three different days,
    // each object has different information in order to store the information for a daily weather forecast.
    ...
    user.calcAge = function() {
        ...
        if (this.d_o_b.getMonth() <= today.getMonth()){
            if (this.d_o_b.getMonth() < today.getMonth() || (this.d_o_b.getMonth() === today.getMonth() && this.d_o_b.getDate() <= today.getDate())) {   //"this" is used inside a method.
