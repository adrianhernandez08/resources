const apiKey = "f94de48a8d3f08bfb3da0b67ab2ed431";
const KELVIN = 273;
...
function checkWeather(latitude, longitude) {
    ...
    let api = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${apiKey}&units=imperial`

    fetch(api)
        .then(response => response.json())
        .then(data => {

            let temp = Math.floor(data.main.temp - KELVIN + 32);
            let city = data.name;
            let feels_like = Math.floor(data.main.feels_like - KELVIN + 32);
